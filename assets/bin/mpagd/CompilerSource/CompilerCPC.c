...
{
	0, 1, 2, 3, 5, 6, 4,		// keys up, down, left, right, fire2, fire3, fire.
...
{
	65,59,33,32,38,71,44,64,65,57,56
};
...
int main( int argc, const char* argv[] )
{
	...
	puts( "AGD Compiler for Amstrad CPC464 version 0.7.10" );
	puts( "(C) Jonathan Cauldwell June 2020" );
	...
	if ( pSource )
	{
		...
		WriteInstructionAndLabel( "WINH   equ WINDOWHGT * 8" );
		WriteInstructionAndLabel( "WINW   equ WINDOWWID * 5" );
	...
	if ( nAdventure > 0 )
	{
		...
		fprintf( pObject, "       ld de,(dispx)\n" );
		fprintf( pObject, "       ld a,(scno)\n" );
		fprintf( pObject, "       ld (hl),a\n" );
		fprintf( pObject, "       inc hl\n" );
		fprintf( pObject, "       ld (hl),e\n" );
		fprintf( pObject, "       inc hl\n" );
		fprintf( pObject, "       ld (hl),d\n" );
		fprintf( pObject, "       inc hl\n" );
		...
		fprintf( pObject, "       ld e,(hl)\n" );
		fprintf( pObject, "       inc hl\n" );
		fprintf( pObject, "       ld d,(hl)\n" );
		fprintf( pObject, "       ld (dispx),de\n" );
		fprintf( pObject, "       inc hl\n" );
		...
		fprintf( pObject, "pbptr  defw eop\n" );
	}
	else
	{
...
{
	CompileArgument();
	WriteInstruction( "ld c,a" );							/* copy accumulator to first colour. */
	WriteInstruction( "ld b,a" );							/* duplicate for second. */
	WriteInstruction( "call 48184" );						/* set border colour. */
}
...
void CR_ValidateMachine( void )
{
	...
	if ( nArg == INS_NUM )									/* first argument is numeric. */
	{
		...
		if ( nArg != FORMAT_CPC )							/* make sure this is the correct machine. */
		{
