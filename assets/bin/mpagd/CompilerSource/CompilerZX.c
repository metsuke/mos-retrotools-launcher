...
int main( int argc, const char* argv[] )
{
	...
	fprintf( pObject, "       org 24832\n\n" );

	if ( nDebug != 0 )
	{
		/* Find the debugger. */
		pDebug = fopen( szDebugFilename, "r" );
		if ( !pDebug )
		{
			fputs( "Cannot find debugzx.asm\n", stderr );
			exit ( 1 );
		}

		/* Copy the engine to the target file. */
		lSize = fread( cChar, 1, 1, pDebug );				/* read first character of debugger source. */

		fprintf( pObject, "       call idebug\n\n" );		/* initialise the debugger. */

		while ( lSize > 0 )
		{
			fwrite( cChar, 1, 1, pObject );					/* write code to output file. */
			lSize = fread( cChar, 1, 1, pDebug );			/* read next byte of debugger code. */
		}
	}
	...
	/* Copy the engine to the target file. */
//	lSize2 = fread( cChar, 1, 1, pEngine );			/* read first character of engine source. */

//	while ( lSize2 > 0 )
//	{
//		fwrite( cChar, 1, 1, pObject );				/* write code to output file. */
//		lSize2 = fread( cChar, 1, 1, pEngine );		/* read next byte of source. */
//	}
