...
#define TITLE_STR "AY Sound FX Editor v0.7"
...
void EffectMakeWAV(int fxn,int off)
{
	...
	int ifrq=MIX_RATE/frame_rate;
	int frames=EffectRealLen(fxn)+3-off;//плюс, чтобы точно отзвучало полностью
	...
	short int* buf=(short int*)(wave+pp);//оффсет после заголовка
	
	int icnt=0;
	ay_init(&ayChip);
	pp=off;

	for(int aa=0;aa<slen;aa++)
	{
...
void __fastcall TFormMain::MImportVGMClick(TObject *Sender)
{
	...
	OpenDialogImport->Filter="VGM file (*.vgm)|*.vgm";
	if(OpenDialogImport->Execute())
	{
		FormPSGChannelSelect->ShowModal();
		if(import_ok)
		{
...
//---------------------------------------------------------------------------
void __fastcall TFormMain::MFrameRateClick(TObject *Sender){
	FormFrameRate->EditFrameRate->Text=IntToStr(frame_rate);
	FormFrameRate->ShowModal();

	if(FormFrameRate->Confirm)
	{
		int n=StrToIntDef(FormFrameRate->EditFrameRate->Text,0);
		if(n>0) frame_rate=n;
	}
}
//---------------------------------------------------------------------------
